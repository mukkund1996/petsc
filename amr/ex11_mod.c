...
static PetscErrorCode ErrorIndicator_Height(PetscInt dim, PetscReal volume, PetscInt numComps, const PetscScalar u[], const PetscScalar grad[], PetscReal *error, void *ctx)
{
    PetscReal      err = 0.;
    PetscInt       j;
...
static PetscErrorCode adaptToleranceFVM(PetscFV fvm, TS ts, Vec sol, VecTagger refineTag, VecTagger coarsenTag, User user, TS *tsNew, Vec *solNew, FILE *cellfile)
{
  ...
  ierr = DMCreateLocalVector(plex, &grad_plot);CHKERRQ(ierr);
  ...
  ierr = DMPlexReconstructGradientsFVM(plex, locX, grad_plot);CHKERRQ(ierr);
  ...
  OutputGrad(grad_plot, stepnum, plex);
...
//  PetscBool printgrad;
//  printgrad = (time==0.0)?PETSC_TRUE:PETSC_FALSE;
  for (c = cStart; c < cEnd; c++) {
    PetscReal             errInd = 0.;
    ...
    /* Getting the adapting criteria as the product of volume and norm of the gradients */
      PetscBool height = PETSC_FALSE;
      PetscOptionsGetBool(NULL, NULL, "-criteria", &height, NULL);
      if(!height) {
          ...
          ierr = ErrorIndicator_Height(dim, cg->volume, user->model->physics->dof, pointVal, pointGrad, &errInd,
                                       user->model->errorCtx);
          CHKERRQ(ierr);
      }
    errArray[c-cStart] = errInd;
//    if(printgrad) printf("%f \n", errInd);
